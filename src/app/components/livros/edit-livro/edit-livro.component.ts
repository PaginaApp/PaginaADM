...
export class EditLivroComponent {
  ...
  public async ngOnInit(): Promise<void> {
    ...
    if (this.livro) {
      // cria um array para cada select
      const autorArray: AutorDTO[] = [this.livro.autor];
      const editoraArray: EditoraDTO[] = [this.livro.editora];

      this.livroForm.patchValue({
        Titulo: this.livro.liv_Titulo,
        Ano: this.livro.liv_Ano,
        Sinopse: this.livro.liv_Sinopse,
        autor: autorArray,
        editora: editoraArray,
        ISBN: this.livro.liv_ISBN,
        categoria: this.livro.categorias,
      });

      console.log('categorias:', this.livro.categorias);
    }
  }
  ...
  public async saveLivro(): Promise<void> {
    ...
    try {
      ...
      const resposta = await this.livroService.updateLivro(
        livro,
        this.livro!.liv_Id
      );
